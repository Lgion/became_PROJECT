...
// alert(`mettre des boutons pour pouvoir r�-afficher left_output & main_section_\n
// `)
...
//ectte evenement d�l�gu� ci-dessous doit pouvoir exploiter des donn�es empil�s dans une variable
...
// AINSI LE onchange SUR LE select[name=matieres] DOIT REP2RER, GR�CE A L'OBJECT req_obj.profs_id, SI LE USER PEUT PRETENDRE CONTINUER POUR  CREER UN COURS
// , OU S'IL DOIT CREER UN PROFESSEUR DANS LA MATIERE CHOISIE AVANT DE CREER LE COURS DANS CETTE MATIERE..
		...
		$(".alphabet").on('mouseover','a',function(e){
				if(typeof $(this).attr('data-data')!="undefined"){
						...
						//factices datas
						let dup="<a href=''>#TAG_</a>",i=0,dup_="<p>Je suis un texte type orem ipsum de pr�sentationJe suis un texte type orem ipsum de pr�sentationJe suis un texte type orem ipsum de pr�sentationJe suis un texte type orem ipsum de pr�sentationJe suis un texte type orem ipsum de pr�sentationJe suis un texte type orem ipsum de pr�sentationJe suis un texte type orem ipsum de pr�sentationJe suis un texte type orem ipsum de pr�sentationJe suis un texte type orem ipsum de pr�sentation</p>"
		...
		$("#main output,#main output .ressource nav.w25,#main output .ressource nav.w25").on("click",".ressource div.w50>div,.ressource nav.w25>div,.ressource nav.w25>button",function(e){
				...
				// alert("Il s'agit ici, d'ouvrir le div.inmodal et d'y ins�rer quelquechose, en fonction de donn�es dans les attributs")
		})
...
//	CETTE CONDITION NEST JAMAIS ATTEINTE, LE span NA JAMAIS LA CLASS .prof PUISQUE TOUS LES MODES DOIVENT JOUER LA TIMELINE
		...
		$section.find('.courses').on('mousemove','._modal .timeline .tl',function(e){
				...
				// console.log(tmp)
				rt.html(tmp)
				// alert("je rep�re la position X et Y du clic dnas le conteneur et je lance toutes les actions qui ont lieu du point pr�sent jusqu'au cette instant cliqu�")
		})
		...
		$section.find('.courses').on('click','._modal .timeline .options>input[type=submit]',function(){
				...
				if(tl_opts_add_rsc.rsc){
						...
						if(bool){
								alert("eofijVous pouvez maintenant ajouter des �l�ments dans la timelineeljfofv\nPour ajouter de nouveaux �l�ments, il faut selectionner la phase � laquelle ils appartiendront")
								$(this).closest('.prof').addClass('addable')
								$(span).find('.timeline .line .void').addClass("nodisplay")
						}		
				}
				else{bool_=false; alert("rbgonorgviVous devez entrer un titre pour la phase, et un instant durant le course (d�but et fin),fivf")
				}
				if(bool_){
						get_tl(span.tl)
		...
		$section.find('.courses').on("click","._modal .r .scrolly ul a",function(){
				...
				switch(cls){
					...
					}else alert("Aucune phase n'a �t� cr�� dns la timeline!\nSinon ce message ne devrai jamais pouvoir s'afficher car si le coursest d�j� termin� la fenetre '.prof' n'est pas senc� pouvoir etre affich�")
					break
					case"show_r":
					...
					}else alert("eoeXCette op�ratio ne prut etre effectu�e que si le cours a d�j� commenc�ofvnonoen")
					break
					default:break
				}
		})
...
function receive_link(event){
		...
		//deux cas :
				...
				//soit il s'agit d'un lien du second type qui fonctionne avec un identifint apr�s le caract�re '=', auquel celui (via un switch) actif la fonction associ�
}
function ancre_type_second(type=false,url=false,object=false){
		let ext="",tmp,obj,obj_,h,m,f
		if(object===false){
				alert("chaque 'case:' doit etre une promesse\nqui lance la fonction (ancre_type_second()) r�cursivement en y ins�rant les parametre type,url et object\nsinonancre_type_second() ne fonctionnera pas pour les appel ne r�sultant d'evts click")
				switch(type){
		...
		if($section.find(".courses ._modal .prof .line .tl,.courses ._modal .students .line .tl").length!=0){
				if(tl.phases)
						tl.phases.forEach(function(i,j){
								...
								tampon[0].data=i
								tampon.onmousemove=function(e){console.log("je rep�re la position X et Y du clic dnas le conteneur, et j'affiche la valeur 'time' de l'instant")}
								TL.append(tampon)
								if(prof){
			...
			// alert('z'+sessionStorage.becamenet)
				...
				console.log(interval+"_   _"+now+" ___ "+now%5)
						...
						if(ending<=0){
								...
								// $.post("/_/endcourse/"+course_footer_btn.dataset.id+"/"+escape(sessionStorage.becamenet),function(xhr){alert("le cours est enregistr� avec succ�s")})
								let url="/_/endcourse/"+course_footer_btn.dataset.id+"/"+escape(sessionStorage.becamenet).replace(/\//g,'|-|')
								$.post(url,function(xhr){if(xhr==1)alert("le cours est enregistr� avec succ�s");else alert('error : '+xhr)})
								console.log(url)
...
function consocket(prof,students,span){
		...
		masocket.on('joined_',function(data){
				if(data.connected)	if($section.find('.students .status .professeur').html()=="")$section.find('.students .status .professeur').append($("<img/>").attr({src:data.img,title:data.pseudo,"data-id":data.id})).append($("<a/>").attr({href:"#",title:"blablabla"}).html('ueniuengIl faurait pouvoir ins�rer le lien du prof ici.ejej'))
				...
				if(prof){
					...
					console.log(span[0].tl_)
						...
						live.find('>button').each(function(i,j){
								...
								if(j.className=="audio"){/*audio:{optional:[{sourceId:s.value}]}*/j.obj={audio:true,video:false};$(j).click(function(){alert("impossible d'acc�der au micro, va savoir pourquoi, ca a march� durant es premier essaie, pi d'un coup plus rien")})}
						...
						live.find('.gum button').click(function(e){
								...
								}else alert("iueieuouzVeuillez d'abord lancer l'enregistrement avant de tenter une pause ou un arr�t svp.fuuenuoehoue")
						})
		...
		masocket.on('endmedialive',function(data){let tmp=span.find('.contain .live .video video[src!=""],.contain .live .audio audio[src!=""]')[0].src="";alert("iufudfuifvFin du m�diaouuoeoue")})
...
//PETIT MANQUE ICI, LA CONDITION SUR LE bool NEST PAS COMPLETE : SI LE timestamp EST TROUVE DANS LE LIVE (partie manquante) ET SI IL EST TROUVE DANS LA tl
								...
								alert("!!! ATTENTION, il y a des donn�es � ins�rer dans la timeline!")
						break
						break
						default:break
				}
...
function fromproflive(data){
		...
		switch(data._){
			...
			if(bool)live.append($("<section/>").html(b_translates.section.courses._modal.live.contenu_v).attr({"class":data._+" w50 pr fr_","data-timestamp":data.timestamp,"data-src":data.value}).append($("<video/>").attr({/*src:data.value,*/controls:true})).append($("<ul/>").append($("<li/>").append($("<i/>").addClass('icon Record')).attr({title:b_translates.section.courses._modal.live.opt_play,"class":"play_video"})).append($("<li/>").append($("<i/>").addClass('icon Help Circle Outline')).attr({title:b_translates.section.courses._modal.live.opt_ask,"class":"ask"}))))
				...
				break;case"live_stream_finished":alert("il faut cr�er un blob amintenant"),liveblob=[]
				break;default:break
		}
